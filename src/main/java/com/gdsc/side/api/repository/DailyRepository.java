...
//    List<DailyMainResponseInterface> findDailiesByDailyId(@Param("user_id") Long user_id);
     ...
     */
    @Query(value = "select d.daily_id, d.title, dd.daily_status " +
            "from daily_dates dd " +
            "join daily d on dd.daily_id = d.daily_id " +
            "where dd.dates like :date and d.user_id = :user_id and dd.daily_status = :dailyStatus order by dd.dates", nativeQuery = true)
